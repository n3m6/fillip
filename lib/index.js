...
function routeFound(routeTable, address){
  ...
  // TODO: Refactor code to send back reply in one loop, instead of two
...
function cacheCheck(req, res, route, thisObject, callback){
  ...
  if(thisObject.routes[route].caching === true){
    ...
    var params = req.params[Object.keys(req.params)[0]];
    var cacheKey = req.sessionID + ':' + route + ':' + params;
...
Fillip.prototype.apicall = function(req, res){
  ...
  cacheCheck(req, res, route, this, 
    ...
    // if cache object is null, indicating that object was not found in cache
    if(Object.keys(cacheObject).length === 0 ) {

      // No cache object was found. Invoking controller to get JSON
      //
      // if there's a controller attached. if not reply with 404
      if(typeof(thisObject.routes[route].controller) === 'function'){
        
        // Controllers should invoke a callback with a json reply
        // We pass request parameters and the callback function to user controller funcs
        thisObject.routes[route].controller(req.params, function _passingJson(jsonObject){
          
          // Now using JSON object passed back from user controller
        ...
        res.json({ 404: 'controller not found' });
      }
